...
const uploadDir = async (mediaDir, cmd) => {
  let all = await fs.readdir(mediaDir)
  let dirs = []
  for (let item of all){
    let full = path.join(mediaDir, item)
    if ((await stat(full)).isDirectory()){
      dirs.push(full)
    }
  }
  for (let [index, dir] of dirs.sort().entries()){

    let count = 0
    let subCount = 0
    let mediaFile = ''
    let subFile = ''
    let subitems = await fs.readdir(dir)
    for (let file of subitems){
      if(file.toLowerCase().endsWith('avi')){
        if (count >= 1){
          log('warn', `More than 1 avi found at ${dir}`)
          break
        } else {
          count += 1
          mediaFile = path.join(dir, file)
        }
      } else if (file.toLowerCase().endsWith('srt')){
          subCount += 1
          subFile = path.join(dir, file)
        }
    }
    if (count === 1){
      let subDetails = {}
      if (subCount === 1){
        subDetails = {subtitles: subFile}
      }
      log('info', `uploading ${mediaFile}`)
      try {
        let result = await uploadVideo(mediaFile, {yes: true, ...subDetails})
        if(result){
          let resumefile = `zabava_resume_${uuid.v4()}`
          await fs.writeFile(resumefile, JSON.stringify(result, null, 2))
          log('error', `Could not upload ${mediaFile} - resume file ${resumefile}`)
        }
      } catch (err){
        log('error', `Could not upload ${mediaFile} - ${err}`)
      }

    } else if (count === 0){
      log('info', `No avi found at ${dir}`)
    }
  }
}
...
const uploadVideo = async (movieFile, cmd) => {
  ...
  } else if (["avi", "mov", "3gp", "3g2", "gpp", "peg", "mpg", "wmv"].includes(movieFile.toLowerCase().substr(movieFile.length - 3))){
...
const resume = async (cmd) => {
  ...
  let result
  try {
    let nixSafeFile = file.replace(/\\/g, '/').slice(data.outputDir.length + 1)
