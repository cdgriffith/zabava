...
class Backblaze extends ProviderInterface {
  ...
  async _request(url, data, method = 'post', headers = {}, options = {}, verbose=false) {
  ...
  async _uploadHelper(file, localDirectory, remoteDirectory){
    ...
    for (let num of [1,2,3]){
      try {
        ...
        this.uploadProcesses[remoteDirectory].completed.push(file)
        this.uploadProcesses[remoteDirectory].not_complete.splice(this.uploadProcesses[remoteDirectory].not_complete.indexOf(file), 1)
        return
      } catch (err) {
        if (num < 3){
          log('warn', `Could not upload ${file}, try ${num}`)
        } else {
  ...
  async uploadDirectory(localDirectory, remoteDirectory) {
    ...
    let todo = files.slice(0)
    let done = []
    this.uploadProcesses[remoteDirectory] = {"completed": done, "not_complete": todo}
    let uploader = (function(me,local,remote){
      return async function (file){await me._uploadHelper(file, local, remote)}
    })(this, localDirectory, remoteDirectory)
    await asyncPool(5, files, uploader)
    return this.uploadProcesses[remoteDirectory]
  }
  ...
  async deleteDirectory(remoteDirectory) {
    ...
    for (const [index, file] of files.entries()) {
      ...
      try {
        ...
        log('warn', `Could not delete ${file.fileName}, retrying`)
        await this._request(`${this.apiUrl}/${this.apiBase}/b2_delete_file_version`, {
          fileName: file.fileName,
          fileId: file.fileId
        })
