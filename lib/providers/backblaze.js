...
class Backblaze extends ProviderInterface {
  ...
  async uploadDirectory(localDirectory, remoteDirectory) {
    ...
    for (const [index, file] of files.entries()) {
      log('debug', `Uploading file ${index+1} of ${files.length}`)
  ...
  async deleteDirectory(remoteDirectory){
    ...
    for (const [index, file] of files.entries()) {
      ...
      try{
        await this._request(`${this.apiUrl}/${this.apiBase}/b2_delete_file_version`, {fileName: file.fileName, fileId: file.fileId})
      } catch (err){
